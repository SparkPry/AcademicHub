...
export default function Courses() {
  ...
  return (
    <section className="relative z-30 py-24 px-6 dark:bg-gradient-to-b from-transparent to-slate-900/50">
      <h2 className="text-5xl md:text-6xl font-bold text-center mb-16  bg-cyan-500  dark:bg-gradient-to-r from-white to-cyan-400 bg-clip-text text-transparent p-4">
        ...
        {courses.map((course, i) => (
          <Link key={i} to="/courses">
            ...
            >
              ...
              <h3 className="text-2xl font-bold mb-4 text-gray-900 dark:text-cyan-400 dark:group-hover:text-cyan-300 transition-colors">
                {course.title}
              </h3>
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {course.tags.map((tag, j) => (
                  <span
                    key={j}
                    className="px-4 py-2 dark:bg-cyan-400/10 border text-gray-900 dark:border-cyan-400/30 rounded-full text-xs dark:text-cyan-400 group-hover:bg-cyan-400/20 transition-colors"
                  >
                    {tag}
                  </span>
                ))}
